...
const TrailDetailsDifficultyField = ({
  ...
  const [trailDifficulty, setTrailDifficulty] =
    useState<DifficultyLevelEnum>(initialTrailDifficulty);
  const formRef = useRef<HTMLFormElement>(null);
  const { register, handleSubmit } = useTrailDifficultyForm();
  const { mutate: updateTrailDifficulty, isPending } = useUpdateHikingTrailField(
    'trailDifficulty',
    trailId,
    setTrailDifficulty
  );
