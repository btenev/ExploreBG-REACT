...
const AllUsersLockUnlockAccount = ({ user, isAdmin, staffId }: Props) => {
  ...
  const onConfirm = () => {
    ...
    if (!isAdmin && currentUserFromTableIsModerator) {
      toast.info('As a MODERATOR, you are not allowed to lock the account of another MODERATOR.');
  ...
  return (
    <>
      ...
      {isClickLockUnlockAccount && (
        <CommonModal>
          <p>
            Are you sure you want to {accountNonLocked ? 'lock' : 'unlock'} <b>{username}</b>&apos;s
            account?
          </p>
