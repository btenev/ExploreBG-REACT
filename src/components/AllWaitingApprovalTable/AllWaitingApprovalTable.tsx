...
import { useGetWaitingApprovalEntities } from '../../hooks/dataHooks/moderation';
import { LoadingSpinner, PaginationControls, ViewDetails } from '../common';
...
const AllWaitingApprovalTable = ({ waitingApprovalCount, staffId }: Props) => {
  ...
  const resultsPerPage = Number(searchParams.get('pageSize')) || DEFAULT_CARDS_PER_PAGE;
  ...
  const validCollections = Object.entries(waitingApprovalCount)
    ...
    .map(([collection, count]) => [collection, count as number] as [CollectionType, number]);
  ...
  return (
    <>
      ...
      {activeCollection === null && <div>Select a collection to view details</div>}
          ...
          <tbody>
            {items.map((p, index: number) => (
              <tr key={p.id}>
                ...
                <td>
                  {(staffId == p.reviewedBy?.id || p.detailsStatus !== StatusEnum.review) && (
                    <Link to={`/moderation/${activeCollection}/${p.id}/review`}>View</Link>
                  )}
                </td>
                <td>
                  {p.images.length > 0 && (
                    <div style={{ display: 'flex', flexDirection: 'column' }}>
                      <span>
                        {`${
                          p.images.filter((i) => i.image_status === StatusEnum.pending).length
                        } - pending`}
                      </span>
                      <span>
                        {`${
                          p.images.filter((i) => i.image_status === StatusEnum.review).length
                        } - review`}
                      </span>
                      <span>
                        {`${
                          p.images.filter((i) => i.image_status === StatusEnum.approved).length
                        } - approved`}
                      </span>
                ...
                {activeCollection === 'trails' && (
                  <td>
                    <div>
                      ...
                      {p.gpxFile?.gpxStatus && p.gpxFile?.gpxStatus !== StatusEnum.pending && (
                        <ViewDetails details={`reviewed by: ${p.gpxFile?.reviewedBy?.username}`} />
                      )}
                    </div>
                  </td>
                )}
