...
import { ActivityDto, useActivityForm } from '../../hooks/formHooks/trailHooks';
...
const TrailDetailsActivityField = ({
  ...
  const [submittedActivity, setSubmittedActivity] = useState<SuitableForEnum[]>(initialActivity);
  const formRef = useRef<HTMLFormElement>(null);
  const { register, handleSubmit, errors } = useActivityForm();
  const { mutate: updateActivity, isPending } = useUpdateHikingTrailField(
    'activity',
    trailId,
  ...
  return (
    <div className="trail__pair__field-wrapper">
      <div className="trail__pair__field-wrapper__ul" style={{ opacity: isVisible ? '0' : '1' }}>
        suitable for:
        {canEdit && (
          <FaEdit
