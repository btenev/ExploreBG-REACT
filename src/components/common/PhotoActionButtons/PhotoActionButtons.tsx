...
import { useDeletePhotos } from '../../../hooks/dataHooks/useDeletePhotos';
...
const PhotoActionButtons = ({ entityId, folder }: Props) => {
  const { photos, photosForDelete, isDeletePhotosClick, changeMainClick, showConfirmationModal } =
    usePhotosState();
  ...
  return (
    <>
      <div className="photos-wrapper__buttons">
            ...
            <button
              onClick={() =>
                photosForDelete.length > 0
                  ? dispatch({ type: 'SET_SHOW_CONFIRMATION_MODAL', payload: true })
                  : (dispatch({ type: 'SET_IS_DELETE_PHOTOS_CLICK', payload: true }),
                    dispatch({ type: 'SET_CHANGE_MAIN_CLICK', payload: false }))
              }
            >
              <span>{photosForDelete.length > 0 ? 'Delete marked' : 'Delete photos'}</span>
              <ImBin style={{ color: 'red' }} />
            </button>

            <button
              onClick={() => {
                dispatch({ type: 'SET_CHANGE_MAIN_CLICK', payload: !changeMainClick });
                dispatch({ type: 'SET_PHOTOS_FOR_DELETE', payload: [] });
                dispatch({ type: 'SET_IS_DELETE_PHOTOS_CLICK', payload: false });
              }}
            >
        ...
        {photos.length < 10 && <UploadPhotos entityId={entityId} folder={folder} />}

        {showConfirmationModal && (
          <ConfirmationModal
            deletionObj={photosForDelete.length == photos.length ? 'all photos' : 'marked photos'}
