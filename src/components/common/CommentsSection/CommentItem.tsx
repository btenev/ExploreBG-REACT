...
const CommentItem = ({
  ...
  const { register, handleSubmit, setValue, errors } = useCommentForm({ message: comment.message });
      ...
      <div className="comments__wrapper__comment__message">
        <p style={{ opacity: isEditing ? '0' : '1' }}>
          ...
          <time dateTime={comment.lastUpdateDate}>
            {comment.lastUpdateDate && formatEntityLastUpdate(comment.lastUpdateDate)}
          </time>
        </p>

        {userId === comment.owner.id && !isEditing && (
          <FaEdit className="trail-edit-icon" onClick={() => setEditingCommentId(comment.id)} />
        )}
        ...
        {isEditing && (
          ...
          >
            <input type="text" {...register('message')} aria-label="Edit comment message" />

            <SubmitButton buttonName="Send" isSubmitting={isSubmitting} />
            <button type="button" onClick={() => setEditingCommentId(null)}>
              Cancel
            </button>
            {setCommentForDelete && <ImBin onClick={() => setCommentForDelete(comment.id)} />}
