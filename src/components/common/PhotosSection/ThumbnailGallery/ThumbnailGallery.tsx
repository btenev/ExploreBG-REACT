...
const ThumbnailGallery = ({ entityId, canEdit }: Props) => {
  ...
  const handleCheckboxClick = (id: number) => {
    dispatch({
      type: "SET_PHOTOS_FOR_DELETE",
      payload: photosForDelete.includes(id)
        ? photosForDelete.filter((p) => p !== id)
        : [...photosForDelete, id],
    });
  };
  ...
  return (
    <>
      {isUploading && (
        ...
        >
          <LoadingSpinner
            ...
            uploadOrDelete={
              photosForDelete.length > 0 ? "Deleting" : "Uploading"
            }
          />
        </div>
      )}
      ...
      {photos.length > 0 && (
        <div
          className="photos-wrapper__photos"
          style={{ marginTop: canEdit ? "2rem" : "0" }}
        >
          {photos.some(
            (p) => p.imageStatus?.toLocaleLowerCase() == "review"
          ) && (
          ...
          {photos
            ...
            .map((p, index) => (
              <div key={p.id} className="photos-wrapper__photos__img">
                {p.isMain && canEdit && (
                  <span className="photos-wrapper__photos__img__span-main">
                    Main
                  </span>
                )}
                ...
                <figure>
                  <img
                    src={p.imageUrl}
                    onClick={() =>
                      setZoomPhoto({ imageUrl: p.imageUrl, index })
                    }
                    width={150}
                    height={150}
                    alt="Trail photo"
                    className={
                      p.isMain && canEdit
                        ? "photos-wrapper__photos__img__main"
                        : ""
                    }
                  />
                </figure>
