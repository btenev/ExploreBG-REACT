...
const TrailMapSection = ({ trailId, gpxFile, canEdit }: Props) => {
  ...
  const [creationDate, setCreationDate] = useState<string>(
    gpxFile?.creationDate ?? ""
  );

  const startTime = trackInfo && formatFullDate(trackInfo.startTime);
  const endTime = trackInfo && formatFullDate(trackInfo.endTime);
  const start =
    startTime?.trim().slice(0, 14) === "1 January 1970" ? "n/a" : startTime;
  const end =
    endTime?.trim().slice(0, 14) === "1 January 1970" ? "n/a" : endTime;
  return (
    <section className="map details-page-section">
      {gpxFile?.gpxStatus === StatusEnum.review && (
          ...
          {trackInfo && (
            <aside className="map__track-info">
              ...
              <span>
                distance: &nbsp;
                <strong>
                  {convertMetersToKmM(Number(trackInfo.distance))}
                </strong>
              </span>
              <span>
                start time: &nbsp;
              ...
              <span>
                moving time: &nbsp;
                <strong>
                  {Number(trackInfo.movingTime) > 0
                    ? trackInfo.movingTime
                    : "n/a"}
                </strong>
              </span>
              <span>
              ...
              <span>
                ...
                <strong>
                  {Number(trackInfo.movingPace) > 0
                    ? trackInfo.movingPace
                    : "n/a"}
                </strong>
              </span>
              <span>
              ...
              <span>
                ...
                <strong>
                  {Number(trackInfo.speedMax) > 0 ? trackInfo.speedMax : "n/a"}
                </strong>
              </span>
              <span>
                average heart rate: &nbsp;
                <strong>
                  {trackInfo.averageHr ? trackInfo.averageHr : "n/a"}
                </strong>
              </span>
              <span>
                average cadence: &nbsp;
                <strong>
                  {trackInfo.averageCadence ? trackInfo.averageCadence : "n/a"}
                </strong>
              </span>
              <span>
                average temperature: &nbsp;
                <strong>
                  {trackInfo.averageTemp ? trackInfo.averageTemp : "n/a"}
                </strong>
              </span>
