...
const TrailDetailsDestinationsField = ({
  ...
  const [destinations, setDestinations] =
    useState<IPlace[]>(initialDestinations);
  const formRef = useRef<HTMLDivElement>(null);
  const { control, handleSubmit } = useDestinationsForm();
  const { mutate: updateDestinations, isPending } =
    useLegacyUpdateHikingTrailField("destinations", trailId, setDestinations);
  ...
  const onSubmit = (data: DestinationsDto) => {
    ...
    if (
      JSON.stringify(newDestinationIds) ===
